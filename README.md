...
Standalone ESP-IDF application that upgrades the ESP32-C6 co-processor on [Elecrow CrowPanel 7" ESP32-P4](https://www.elecrow.com/crowpanel-advanced-7-0-inch-esp32-p4-display.html) boards to esp\_hosted v2.12.13 — over the existing SDIO bus, without soldering. Factory firmware is v2.3.0 on V1.0 panels and v2.12.3 on the V1.1 panels currently being sold; both are upgraded.

> **Why v2.12.13?** Normally this target equals the ESP-Hosted *host* version baked into the Arduino ESP32 core the clock is built with, because a slave **older** than the host is the mismatch the driver warns about on every boot. Core 3.3.11 carries host 2.12.11, so parity would mean 2.12.11.
>
> It is deliberately one release ahead as of 19 Sep 2026. On a V1.1 panel with matched 2.12.11 on both sides, the host's SDIO receive path stops permanently partway through a boot — association and DHCP succeed, then nothing comes back at all, including control RPCs that carry no network traffic. Upstream 2.12.12 lists *"fixed sdio incorrect identification of all-ones bus read"*, and an all-ones read is what a host sees when the slave is not driving the line; misreading it as data would desync the receive stream for good. v2.12.13 is the first release carrying that fix with a published binary — there is no `esp32c6-v2.12.12.bin`.
>
> Ahead of the host is the safe direction, and the app already refuses to downgrade. If this does not fix it, the fix was host-side and only a newer Arduino core can deliver it. Check the core you build against: `ESP_HOSTED_VERSION_*` in `esp_hosted_host_fw_ver.h`.
...
- **Not every board still ships with v2.3.0.** A V1.1 panel bought in 2026 arrived with C6 v2.12.3. That is older than the v2.12.13 here, so it does get upgraded. (While this tool targeted v2.11.6, that same board was correctly *skipped* as already newer.) The app compares versions and refuses to downgrade, so "it did nothing" is a valid outcome whenever the C6 is at or past the target.
...
5. Activates new firmware on C6; C6 reboots into v2.12.13
...
| Target C6 firmware | esp\_hosted v2.12.13 |
| ESP-IDF | v5.5.1 (esp\_hosted v2.12.13 requires >= 5.3) |
| esp\_hosted component | v2.12.13 ([ESP Component Registry](https://components.espressif.com/components/espressif/esp_hosted/versions/2.12.13)) |
...
- **C6 firmware binary** — `network_adapter.bin` for ESP32-C6, esp\_hosted v2.12.13 (see below)
...
- `App version:` — should show `2.12.13` (or whichever version you built)
...
```bash
just set-target            # downloads managed components (esp_hosted v2.12.13, littlefs)
...
If you see `[PASS] C6 is at v2.12.13, at or past the v2.12.13 this tool carries — OTA not needed`, it was already upgraded. Nothing more to do.

### 6. Restore your normal firmware

Flash your regular firmware back to the P4. The C6 retains v2.12.13 independently.
...
I esp_hosted: Slave FW version [2.12.13]
```

If your host firmware is built with Arduino ESP32 core 3.3.11, its host version is 2.12.11, so the slave is deliberately one release ahead and the two will **not** read as matched. That is the intended state — see the "Why v2.12.13?" note at the top of this README. The warning to care about is the opposite one, a slave older than the host.

**WiFi stability test:** Connect to your network and confirm WiFi remains connected beyond four minutes. The v2.3.0 failure mode was a hard disconnect at ~4 minutes with no recovery. With v2.12.13, WiFi should remain connected indefinitely.
...
Running this tool a second time is safe. If the C6 already runs v2.12.13 (or anything newer), the app detects it and halts without writing anything.
...
│   ├── idf_component.yml               esp_hosted ==2.12.13, littlefs
...
- **Target firmware: v2.12.13.** Other esp\_hosted versions are untested. The download URL above is specific to v2.12.13. The transfer has been exercised from v2.3.0 to v2.11.6 and from v2.12.3 to v2.12.11; the v2.12.13 target itself has not been run on hardware yet.
...
Yes. If the C6 already runs v2.12.13, the app detects it and halts without writing. Running it again is safe.

**Can I upgrade to a different version (not v2.12.13)?**
...
- [esp\_hosted v2.12.13 on Component Registry](https://components.espressif.com/components/espressif/esp_hosted/versions/2.12.13) — managed component used by this project
